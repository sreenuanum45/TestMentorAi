...
export default async function DashboardPage({
  ...
  const [studyCount, mockCount, allActivity, chartActivity] = await Promise.all([
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-6 py-10">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 p-6 dark:from-blue-950/40 dark:to-indigo-950/40 sm:p-8">
        <div className="relative z-10 max-w-md">
          <p className="text-sm text-neutral-500">Welcome back,</p>
          <h1 className="break-all bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-3xl font-extrabold text-transparent sm:text-4xl">
            {user.email}
          </h1>
      ...
      <div className="mt-6 flex gap-3">
        <Link
          href="/study"
          className="flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-2 text-sm font-medium text-white"
        >
          Continue studying <span aria-hidden>→</span>
        </Link>
        <Link
          href="/mock-interview"
        ...
        >
          <span aria-hidden>🎤</span> Start a mock interview
        </Link>
      </div>
    </div>
  );
